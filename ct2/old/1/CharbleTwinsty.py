...
def focusCameraOn( objpos, orientationq, distancevec ):
    ...
    # Does this need to be normalized?  Maybe.
    #for x in matrix:
    #    x.normalize()
    objvectorx = matrix[0]
    objvectory = matrix[1]
    ...
    # vector
    posvec = objpos.clone()
    posvec.addVector( objvectorx )
    posvec.addVector( objvectory )
    posvec.addVector( objvectorz )


    # It works!
    gluLookAt( posvec.x, posvec.y, posvec.z, \
...
def drawStuff( target, things, effects ):
    ...
    offsetvec = Vector3D( 0.0, 0.0, 5.0 )
    ...
    # We move and rotate, draw the object, then move and rotate back.
    #glTranslate( TARGETLOC.x, TARGETLOC.y, TARGETLOC.z )
    #theta, vec = TARGETQ.toAxis()
    #glRotatef( theta, vec.x, vec.y, vec.z )
    #drawReferenceObject( 1.0 )
    #glRotatef( -theta, vec.x, vec.y, vec.z )
    #glTranslate( -TARGETLOC.x, -TARGETLOC.y, -TARGETLOC.z )

    # Then we go elsewhere and draw another object
